...
#define VMX_TRUE_CTLS_FLAG			0x800000
#define VMX_PROCBASED_SEC_FLAG			0x80000000 
 ...
 */
struct capability_info exit_ctrl[13] =
{
...
detect_vmx_features(void)
{
	...
	if (hi & VMX_TRUE_CTLS_FLAG) // 23rd bit value set to 1
	{
		//True Pin Based Control
		...
		//True Proc Based Control
		rdmsr(IA32_VMX_TRUE_PROCBASED_CTLS, lo, hi);

		pr_info("\nTrue Procbased Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));

		report_capability(procbased, 21, lo, hi);

		if (hi & VMX_PROCBASED_SEC_FLAG) // 31st bit value set to 1
		{	
			// Secondary Proc Based Control
			rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);

			pr_info("\nProcbased Secondary Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		...
		pr_info("\nTrue Exit Control MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));

		report_capability(exit_ctrl, 13, lo, hi);

		//True ENTRY Control
		rdmsr(IA32_VMX_TRUE_ENTRY_CTLS, lo, hi);

		pr_info("\nTrue Entry Control MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
